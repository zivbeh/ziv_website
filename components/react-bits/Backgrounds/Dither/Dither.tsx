...
const RetroEffect = forwardRef<RetroEffectImpl, { colorNum: number; pixelSize: number }>((props, ref) => {
  ...
  const WrappedRetroEffect = wrapEffect(RetroEffectImpl) as unknown as React.ComponentType<{
    colorNum: number;
    pixelSize: number;
    ref?: React.ForwardedRef<RetroEffectImpl>;
  }>;
...
function DitheredWaves({
  ...
  useFrame(({ clock }) => {
    ...
    // Keep mouse wake fully off unless explicitly enabled — never leave a stale "1"
    u.enableMouseInteraction.value = enableMouseInteraction ? 1 : 0;
    u.mouseRadius.value = enableMouseInteraction ? mouseRadius : 0;
    if (enableMouseInteraction) {
      u.mousePos.value.copy(mouseRef.current);
    } else {
      u.mousePos.value.set(0, 0);
    }
  });
